use crate::{
    Block, CommandHistory, Document, EditorCommand, HistoryEntry, Inline, ListItem, Position,
    Selection, Snapshot, Style, TableEditor,
};
use std::sync::Arc;
use uuid::Uuid;
...
impl Editor {
    ...
    pub fn execute(&mut self, cmd: EditorCommand) {
        match cmd.clone() {
            ...
            EditorCommand::InsertList(ordered) => {
                self.history
                    .push_entry(HistoryEntry::Snapshot(self.snapshot()));
        ...
        F: FnMut(&mut Block),
    {
        ...
        if let Some(pos) = self.doc.blocks.iter().position(|b| b.id() == block_id) {
            ...
            let selection_after = self.selection;
            self.history
                .push_or_merge_block_change(HistoryEntry::BlockChange {
                    block_id,
                    before,
                    after,
                    selection_before,
    ...
    fn insert_text_into_block(block: &mut Block, text: String) {
        if let Block::Paragraph { content, dirty, .. } | Block::Heading { content, dirty, .. } =
            block
        {
