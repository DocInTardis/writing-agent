use crate::{
    FontMetrics, FontdueMeasurer, ImageCache, LayoutCache, LineBreaker, RealMeasurer,
    SharedMeasurer,
};
use lru::LruCache;
...
impl LayoutEngine {
            ...
            let lb = if dirty {
                if let Some(hit) = cache.get(block.id()) {
                    if cache.signature(block.id()) == Some(sig) {
                        ...
                        let fresh =
                            std::sync::Arc::new(self.layout_block_with_pool(block, config, cache));
    ...
    #[cfg(feature = "parallel")]
    fn layout_cached_parallel(
        &mut self,
        doc: &Document,
        config: &LayoutConfig,
        cache: &mut LayoutCache,
    ) -> LayoutTree {
        let mut reuse: Vec<Option<std::sync::Arc<LayoutBlock>>> =
            Vec::with_capacity(doc.blocks.len());
    ...
    fn layout_block_inner(
        ...
        match block {
            Block::Heading { level, content, .. } => {
                ...
                let lines =
                    self.wrap_text_with_pool(&text, width, config.metrics, cache.as_deref_mut());
                let height =
                    lines.len() as f32 * config.metrics.font_size * config.metrics.line_height;
                LayoutBlock {
                    block_id: block.id(),
            ...
            Block::List { items, .. } => {
                let mut lines =
                    self.alloc_lines(cache.as_deref_mut(), items.len().saturating_mul(2));
                for (idx, item) in items.iter().enumerate() {
                    ...
                    let _ =
                        std::fmt::Write::write_fmt(&mut self.scratch, format_args!("{} ", idx + 1));
                    join_inline_into(&mut self.scratch, &item.content);
                    let text = std::mem::take(&mut self.scratch);
                    let wrapped = self.wrap_text_with_pool(
                        &text,
                        width,
                        config.metrics,
                        cache.as_deref_mut(),
            ...
            Block::Quote { content, .. } => {
                let mut lines =
                    self.alloc_lines(cache.as_deref_mut(), content.len().saturating_mul(2));
    ...
    fn maybe_log_stats(&self) {
        ...
        eprintln!(
            "[layout] break_cache hit_rate={:.2} hits={} misses={}",
            hit_rate, self.break_cache_hits, self.break_cache_misses
        );
