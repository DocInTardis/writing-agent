...
def _find_exe(name: str) -> Path | None:
    configured = str(os.environ.get("WA_RUST_ENGINE_BIN_DIR", "") or "").strip()
    roots = [Path(configured)] if configured else []
    roots.extend(
        [
            PACKAGE_BIN_DIR,
            REPO_ROOT / "engine" / "target" / "release",
            REPO_ROOT / "engine" / "target" / "debug",
        ]
    )
    names = [name]
