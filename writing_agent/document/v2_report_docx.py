...
from writing_agent.document import v2_report_docx_helpers as docx_helpers, v2_report_docx_toc as toc_support
...
def _docx_bool_env(name: str, default: bool = False) -> bool:
...
def _docx_float_env(name: str, default: float) -> float:
    try:
...
def _docx_should_include_figure(score_row: dict[str, Any] | None) -> bool:
    row = score_row if isinstance(score_row, dict) else {}
    min_score = max(0.0, min(100.0, _docx_float_env("WRITING_AGENT_DOCX_FIGURE_MIN_SCORE", 60.0)))
    allow_review = _docx_bool_env("WRITING_AGENT_DOCX_FIGURE_ALLOW_REVIEW", False)
    ...
    if recommendation == "drop":
        return False
    if recommendation == "review" and not allow_review:
        return False
    return True
...
class V2ReportDocxExporter:
    ...
    def build_from_parsed(
        ...
        *,
        ...
        if template_path:
            ...
            heading_styles = docx_helpers._detect_template_heading_styles(doc)
            docx_helpers._truncate_template_body(doc)
        else:
            doc = Document()
        self._apply_page_setup(doc, prefs)
        self._apply_styles(doc, formatting)
        if _update_fields_on_open_enabled():
            docx_helpers._enable_update_fields(doc)
        else:
            docx_helpers._disable_update_fields(doc)

        raw_title = docx_helpers._strip_inline_markers((parsed.title or "").strip())
        if raw_title in {"\u672a\u547d\u540d\u6587\u6863", "\u672a\u547d\u540d\u62a5\u544a"}:
            raw_title = ""
        title = docx_helpers._sanitize_heading_text(raw_title or "\u62a5\u544a") or "\u62a5\u544a"
        blocks = list(parsed.blocks or [])
        blocks = docx_helpers._remove_disallowed_sections(blocks)
        # 优化: 移除虚拟图表插入,让模型自然生成
        # blocks = _ensure_min_figures(blocks)
        # blocks = _ensure_min_tables(blocks)
        blocks = docx_helpers._ensure_reference_section(blocks)
        # Normalize heading structure before TOC/content emission.
        if blocks and blocks[0].type == "heading" and int(blocks[0].level or 0) == 1:
            blocks = blocks[1:]
        blocks = docx_helpers._promote_headings_if_no_h1(blocks)
        mojibake_hits = docx_helpers._collect_export_mojibake_fragments(title, blocks)
        if mojibake_hits:
    ...
    def _apply_page_setup(self, doc: Document, prefs: ExportPrefs) -> None:
        ...
        top = docx_helpers._pick_margin(prefs.page_margin_top_cm, m + 0.5)
        bottom = docx_helpers._pick_margin(prefs.page_margin_bottom_cm, m)
        left = docx_helpers._pick_margin(prefs.page_margin_left_cm, m + 0.5)
        right = docx_helpers._pick_margin(prefs.page_margin_right_cm, max(1.5, m - 0.5))
    ...
    def _collect_toc_entries(self, blocks: list[DocBlock], *, levels: int) -> list[tuple[int, str]]:
        return toc_support.collect_toc_entries(blocks, levels=levels)

    def _build_toc_anchor_entries(self, entries: list[tuple[int, str]]) -> list[TocAnchorEntry]:
        return toc_support.build_toc_anchor_entries(entries, entry_factory=TocAnchorEntry)

    def _display_width(self, text: str) -> int:
        return toc_support.display_width(text)

    def _estimate_toc_pages(self, entries: list[tuple[int, str]], blocks: list[DocBlock], *, start_page: int = 1) -> list[int]:
        return toc_support.estimate_toc_pages(entries, blocks, start_page=start_page)

    def _render_toc_preview_text(self, entries: list[tuple[int, str]], blocks: list[DocBlock]) -> str:
        return toc_support.render_toc_preview_text(entries, blocks)
    ...
    def _add_toc(
        ...
        *,
        ...
        toc_support.add_toc(
            ...
            entries=entries,
            blocks=blocks,
            resolve_toc_style_name=self._resolve_toc_style_name,
            dynamic_toc_field_enabled=_dynamic_toc_field_enabled(),
            toc_hyperlink_enabled=_toc_hyperlink_enabled(),
            toc_field_lock_enabled=_toc_field_lock_enabled(),
            toc_clickable_links_enabled=_toc_clickable_links_enabled(),
        )
    ...
    def _emit_content(
        ...
        *,
        ...
        has_reference_heading = any(
            b.type == "heading"
            and docx_helpers._is_reference_title(
                docx_helpers._normalize_section_title(docx_helpers._sanitize_heading_text((b.text or "").strip()))
            )
            for b in blocks
        )
        ...
        def _attach_anchor(paragraph: Paragraph | None, level: int, heading_title: str) -> None:
            ...
            if not anchor:
                return
            docx_helpers._add_bookmark(paragraph, anchor, bookmark_id)
            bookmark_id += 1
        ...
        def handle_paragraph_text(raw_text: str) -> None:
            ...
            for chunk in docx_helpers._split_paragraph_chunks(raw_text):
                t = docx_helpers._sanitize_paragraph_text(chunk)
                if not t:
                    continue
                bullet = re.match(r"^\s*[\u2022\u00B7]\s+(.+)$", t)
                num_bullet = re.match(r"^\s*\d+[.\uFF0E\u3001\)]\s*(.+)$", t)
                is_ref_line = bool(re.match(r"^\s*\[\d+\]\s+", t))
                if is_ref_line and not docx_helpers._is_reference_title(current_section):
                    if not inserted_reference_heading:
                        ref_title = "\u53c2\u8003\u6587\u732e"
                        if number_headings:
                            docx_helpers._add_heading(doc, ref_title, level=1, align=WD_ALIGN_PARAGRAPH.CENTER)
                        else:
                            ref_level = 2
                            style_name = heading_styles.get(ref_level) if heading_styles else None
                            docx_helpers._add_heading(doc, ref_title, level=ref_level, style_name=style_name)
                        current_section = ref_title
                        ref_index = 0
                        inserted_reference_heading = True
                        has_reference_heading = True
                if docx_helpers._is_reference_title(current_section) and docx_helpers._is_reference_noise(t):
                    continue
                if docx_helpers._is_reference_title(current_section):
                ...
                if num_bullet:
                    ...
                    try:
                        ...
                        docx_helpers._add_inline_runs(p, item)
                    except KeyError:
                        p = doc.add_paragraph()
                        docx_helpers._add_inline_runs(p, f"{item}")
                    if apply_para_format:
                        p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
                        try:
                            p.paragraph_format.space_before = Pt(0)
        ...
        for b in blocks:
            if b.type == "heading":
                ...
                text, tail = docx_helpers._split_heading_tail(raw_text)
                if not text:
                    continue
                if docx_helpers._is_reference_title(current_section) and ref_buffer:
                    flush_reference_buffer()
                if number_headings:
                    clean = docx_helpers._strip_chapter_prefix(text)
                    if docx_helpers._is_reference_title(docx_helpers._normalize_section_title(clean)):
                        if seen_h1:
                            doc.add_page_break()
                        seen_h1 = True
                        heading_paragraph = docx_helpers._add_heading(doc, clean, level=1, align=WD_ALIGN_PARAGRAPH.CENTER)
                        _attach_anchor(heading_paragraph, lvl, text)
                        current_section = docx_helpers._normalize_section_title(clean)
                        ref_index = 0
                        continue
                    if lvl <= 1:
                        if seen_h1:
                        ...
                        h3 = 0
                        t = f"\u7b2c{h1}\u7ae0 {clean}"
                        heading_paragraph = docx_helpers._add_heading(doc, t, level=1, align=WD_ALIGN_PARAGRAPH.CENTER)
                        _attach_anchor(heading_paragraph, lvl, text)
                    elif lvl == 2:
                        if h1 == 0:
                            h1 = 1
                        ...
                        h3 = 0
                        t = f"{h1}.{h2} {clean}"
                        heading_paragraph = docx_helpers._add_heading(
                            doc,
                            t,
                            level=2,
                        ...
                        h3 += 1
                        t = f"{h1}.{h2}.{h3} {clean}"
                        heading_paragraph = docx_helpers._add_heading(doc, t, level=3, align=WD_ALIGN_PARAGRAPH.LEFT)
                        _attach_anchor(heading_paragraph, lvl, text)
                else:
                    ...
                    if heading_styles:
                        ...
                        heading_paragraph = docx_helpers._add_heading(doc, text, level=level, style_name=style_name)
                    else:
                        heading_paragraph = docx_helpers._add_heading(doc, text, level=level)
                    _attach_anchor(heading_paragraph, level, text)
                current_section = docx_helpers._normalize_section_title(text)
                if docx_helpers._is_reference_title(current_section):
                    ref_index = 0
                if tail:
            ...
            if b.type == "figure":
                ...
                try:
                    ...
                    continue
                caption = str(rendered_caption or f.get("caption") or "").strip()
                scored_figure = dict(f)
                if caption:
                    scored_figure["caption"] = caption
                score_row = score_figure_spec(scored_figure, svg=svg, png_rendered=bool(png))
                if not _docx_should_include_figure(score_row):
                    continue
                fig_no += 1
                caption = caption or f"图{fig_no}"
