<script lang="ts">
  ...
  function handleToolbarState(event: any) {
    ...
    editorToolbarState = {
      ...
      inTable: Boolean((detail as any).inTable),
      canMergeCells: Boolean((detail as any).canMergeCells),
      canSplitCell: Boolean((detail as any).canSplitCell),
      tableCaption: String((detail as any).tableCaption || ''),
      tableRepeatHeader: Boolean((detail as any).tableRepeatHeader),
      tableAlignment: String((detail as any).tableAlignment || 'left'),
      tableWidthPercent: Number((detail as any).tableWidthPercent || 100),
      tableCellBackground: String((detail as any).tableCellBackground || ''),
      tableCellVerticalAlign: String((detail as any).tableCellVerticalAlign || 'top'),
