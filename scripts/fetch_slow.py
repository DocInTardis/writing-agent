"""Slow fetcher with conservative rate limits to avoid arXiv 429."""
...
def fetch_with_retry(query: str, limit: int) -> list[dict[str, Any]]:
    for attempt in range(3):
        papers = fetch_arxiv_ids(query, limit=limit)
        if papers:
            return papers
        wait = 30 * (attempt + 1)
        logger.info("Retry %d/3 for '%s', waiting %ds...", attempt + 1, query, wait)
        time.sleep(wait)
    return []
...
def main() -> None:
    ...
    for query in queries:
        ...
        papers = fetch_with_retry(query, limit=10)
        if not papers:
            logger.warning("Skipping: %s", query)
            continue
        for paper in papers:
            added = process_paper(paper, store, kg, extractor, processed_ids)
            total_added += added
            time.sleep(3.0)  # conservative: 3s per paper
        logger.info("Progress: %d papers | %d KUs", len(processed_ids), total_added)
        time.sleep(10.0)  # conservative: 10s between queries
