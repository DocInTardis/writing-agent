<script lang="ts">
  ...
  let qualityAdviceItems: QualityAdviceItem[] = []
  let qualityOverview: QualityOverview = {
    tone: 'good',
    label: '待检测',
    note: '尚未运行质量检测'
  }
  ...
  $: qualityOverview = buildQualityOverview()
  $: qualityAdviceItems = buildQualityAdviceItems()
        ...
        {#if showAdvancedToolbar}
          <div class="toolbar-line secondary">
            <div class="toolbar-cluster">
              ...
              <button class="tool-btn" title="重做 Ctrl/Cmd+Y" aria-label="重做" onclick={() => runEditorCommand('redo')} disabled={!editorToolbarState.canRedo}>
